...
$where = [];
if ($bulan) $where[] = "MONTH(pg.tanggal)='$bulan'";
if ($tahun) $where[] = "YEAR(pg.tanggal)='$tahun'";
$where_sql = $where ? 'WHERE ' . implode(' AND ', $where) : '';

$query = "SELECT 
    t.id_tagihan, p.nama, MONTH(pg.tanggal) AS bulan, YEAR(pg.tanggal) AS tahun, (pg.meter_akhir - pg.meter_awal) AS pemakaian, tf.tarif_per_kwh, t.jumlah_tagihan, t.status
  FROM tagihan t
  ...
  $where_sql
  ORDER BY YEAR(pg.tanggal) DESC, MONTH(pg.tanggal) DESC";
...
<table border="1">
  ...
  <?php if ($data && mysqli_num_rows($data) > 0): ?>
    ...
    <?php endwhile; ?>
  <?php else: ?>
    <tr><td colspan="7" align="center">Tidak ada data tagihan.</td></tr>
  <?php endif; ?>
</table> 